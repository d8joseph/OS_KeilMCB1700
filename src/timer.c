...
MEMORY_BLOCK g_timer_signal;

/**
 * @brief: initialize timer. Only timer 0 is supported
 */
U32 timer_init(uint8_t n_timer)
{
	...
	*/
	...
	((MSGBUF*) g_timer_signal.mp_shared_mem)->mtype = TIMER_SIGNAL_STATIC_MEM;
	((MSGBUF*) g_timer_signal.mp_shared_mem)->mtext[0] = 0;
...
{
	MSGBUF* p_static_signal = (MSGBUF*) g_timer_signal.mp_shared_mem;
	...
	if (p_static_signal->mtext[0]) {	// means it's still in use
		MSGBUF* p_block = k_request_memory_block_custom(0);
		if (p_block != NULL) {
			p_block->mtype = DEFAULT;
			k_send_message_proxied(PID_I_TIMER, PID_I_TIMER, p_block);
		}
	} else {
		p_static_signal->mtext[0] = 1;
		k_send_message_proxied(PID_I_TIMER, PID_I_TIMER, p_static_signal);
	}
}
